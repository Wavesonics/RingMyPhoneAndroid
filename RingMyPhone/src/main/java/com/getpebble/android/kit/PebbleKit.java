...
public final class PebbleKit
{
	                                      ...
	                                      final String name, final Bitmap icon ) throws IllegalArgumentException
	{
		...
		if( name.length() > NAME_MAX_LENGTH )
		{
			throw new IllegalArgumentException( String.format(
					                                                 "app name exceeds maximum length (%d)",
					                                                 NAME_MAX_LENGTH ) );
		}
	...
	public static boolean isWatchConnected( final Context context )
	{
		Cursor c =
				context.getContentResolver().query(
						                                  Uri.parse( "content://com.getpebble.android.provider/state" ), null,
						                                  null,
						                                  null, null );
			...
			throws IllegalArgumentException
	{
		...
		{
			throw new IllegalArgumentException( String.format(
					                                                 "transaction id must be between (0, 255); got '%d'",
					                                                 transactionId ) );
		}
	...
	public static abstract class PebbleDataLogReceiver extends BroadcastReceiver
	{
		                                      ...
		                                      final UnsignedInteger timestamp, final UnsignedInteger tag )
		{
			...
			final PebbleDataType type =
					PebbleDataType.fromByte( intent.getByteExtra( PBL_DATA_TYPE, PebbleDataType.INVALID.ord ) );
			if( type == null )
			{
				throw new IllegalArgumentException();
			}
